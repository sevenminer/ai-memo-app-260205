...
const sampleMemos: Omit<Memo, 'id' | 'createdAt' | 'updatedAt'>[] = [
  {
    title: '프로젝트 회의 준비',
    content:
...
export async function seedSampleData(): Promise<{ success: boolean; message: string }> {
  ...
  if (existingData && existingData.length > 0) {
    return {
      success: false,
      message: '이미 데이터가 존재합니다. 시딩을 건너뜁니다.',
    }
  }
  ...
  const memosWithDates = [
    ...
    },
    {
      ...
      updated_at: getDateString(8),
    },
    {
      ...sampleMemos[4],
      created_at: getDateString(15),
      updated_at: getDateString(15),
    },
    {
      ...sampleMemos[5],
      created_at: getDateString(20),
      updated_at: getDateString(12),
    },
  ]
  ...
  const { error: insertError } = await supabase
    .from('memos')
    .insert(memosWithDates)

  if (insertError) {
    console.error('Error seeding data:', insertError)
    return {
      success: false,
      message: '데이터 시딩 중 오류가 발생했습니다.',
    }
  }
